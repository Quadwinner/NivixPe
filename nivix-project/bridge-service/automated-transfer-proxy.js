...
const PORT = process.env.AUTOMATED_TRANSFER_PORT || 3011;
...
app.post('/automated-transfer', async (req, res) => {
  try {
    ...
    // 3) Initiate payout via bridge (Cashgram inside bridge)
...
// Poll payout status via bridge and surface Cashgram redemption state
app.get('/automated-transfer/status', async (req, res) => {
  try {
    const { payoutId } = req.query;
    if (!payoutId) return res.status(400).json({ success: false, error: 'payoutId is required' });

    const statusResp = await axios.get(`${BRIDGE_BASE}/api/offramp/status/${encodeURIComponent(payoutId)}`, { timeout: 15000 });
    const data = statusResp.data?.transaction || statusResp.data;

    // Map Cashgram statuses to simple states
    let state = data?.status || 'PROCESSING';
    const cashgram = data?.cashgram_status || data?.provider_response?.cashgram_status;
    if (cashgram) {
      if (/REDEEMED|SUCCESS/i.test(cashgram)) state = 'COMPLETED';
      else if (/EXPIRED|CANCELLED|FAILED/i.test(cashgram)) state = 'FAILED';
      else state = 'PENDING_REDEMPTION';
    }

    return res.json({ success: true, state, raw: data });
  } catch (error) {
    const status = error.response?.status;
    const data = error.response?.data;
    return res.status(status || 500).json({ success: false, error: data?.message || error.message, data });
...
app.listen(PORT, () => {
  console.log(`Automated Transfer proxy listening on http://localhost:${PORT}`);
});
